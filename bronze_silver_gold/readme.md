...
- **Operations**: 
  - **MERGE** to handle updates, deduplication by `claim_id`.
- **Quality Checks**:
...
- The Bronze layer uses `mergeSchema=true` to handle evolving source schemas without pipeline breaks.
...
- The Silver layer blocks bad data from reaching Gold, logging quality failures to a separate **DQ table**.
...
- Use **OPTIMIZE** with a file size target (128MB-256MB per file).
